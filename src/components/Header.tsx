...
interface Props {
  open: boolean;
  handlerShowQuizzModal : () => void;
};

const Header = ({ handlerShowQuizzModal, open }: Props) => {
  ...
  return (
    <Layout theme={theme}>
      ...
      <UserWrapper>
        ...
        {user && questionsAvailable && <Button onClick={handlerShowQuizzModal}>{open ? 'Hide' : 'Show'} quizz</Button>}
      </UserWrapper>
    </Layout>);
}
